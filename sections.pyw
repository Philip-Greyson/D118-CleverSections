...
if __name__ == '__main__':  # main file execution
    with open('sections_log.txt', 'w') as log:
        ...
        with open(OUTPUT_FILE_NAME, 'w') as output:  # open the output file
            print('"School_id","Section_id","Teacher_id","Teacher_2_id","Teacher_3_id","Teacher_4_id","Teacher_5_id","Teacher_6_id","Teacher_7_id","Teacher_8_id","Teacher_9_id","Teacher_10_id","Course_name","Course_number","Section_number","Period","Subject","Term_start","Term_end","Term_name"', file=output)  # print out the header row
            try:
                with oracledb.connect(user=DB_UN, password=DB_PW, dsn=DB_CS) as con:  # create the connecton to the database
                    with con.cursor() as cur:  # start an entry cursor
                        ...
                        for school in schools:
                            ...
                            try:
                                if yearID:
                                    cur.execute('SELECT id, firstday, lastday, schoolid, yearid, abbreviation FROM terms WHERE schoolid = :school AND yearid = :year ORDER BY id', school=schoolID, year=yearID)
                                    terms = cur.fetchall()
                                    for term in terms:
                                        ...
                                        for section in sections:
                                            ...
                                            gradeLevel = int(section[3]) if section[3] else 'Other'
